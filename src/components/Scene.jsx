...
// --- THE CIRCULAR FLOATING PLATFORM & WHOLESOME SCENE ---
const FloatingPlatformWithScene = ({ butterProps }) => {
  return (
    <Float speed={1.5} rotationIntensity={0.2} floatIntensity={0.5}>
      <group position={[10, -1.1, 16]}>
        {/* The Glass/White Base */}
        ...
        <group position={[0, 0.1, 0]}>
          ...
          <group position={[-0.6, 0.1, 0]}>
            <mesh castShadow><boxGeometry args={[0.9, 0.25, 0.9]} /><meshStandardMaterial color="#f3d9ff" /></mesh>
            <mesh position={[0, 0.6, -0.35]} castShadow><boxGeometry args={[0.9, 0.9, 0.2]} /><meshStandardMaterial color="#f3d9ff" /></mesh>
          </group>

          {/* Elder sitting comfortably */}
          ...
          {/* Helper with platter */}
          <group position={[0.9, 0.1, 0.6]} rotation={[0, -0.5, 0]}>
            <BlockHumanoid
              isHelper
              scale={0.8}
              materialProps={butterProps}
...
export default function Scene({ currentView }) {
  ...
  return (
    <>
      ...
      {/* RENDERED LAST - CIRCLE PLATFORM WITH COZY SCENE */}
      <FloatingPlatformWithScene butterProps={butterProps} />
    </>
  );
}