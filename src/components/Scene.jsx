...
const HomeLamp = ({ position }) => (
  <group position={position}>
    ...
    {/* L-Bend Arm */}
    <mesh position={[0.4, 3.0, 0]} rotation={[0, 0, Math.PI / 2]} renderOrder={10001}>
      <cylinderGeometry args={[0.04, 0.04, 0.8, 16]} />
      <meshBasicMaterial color="#333" depthTest={false} />
    </mesh>
    {/* Standard White Shade */}
    <mesh position={[0.8, 2.7, 0]} renderOrder={10002}>
    ...
    <mesh position={[0.8, 2.7, 0]} renderOrder={10003}>
      <sphereGeometry args={[0.12, 16, 16]} />
      <meshStandardMaterial color="#fff" emissive="#fff" emissiveIntensity={1.5} depthTest={false} />
    </mesh>
    {/* Light source logic (visual bloom) */}
    <pointLight position={[0.8, 2.6, 0]} intensity={1} distance={10} color="#fff" />
  </group>
);
...
const FloatingPlatform = ({ butterProps }) => {
  return (
    <Float speed={1.8} rotationIntensity={0.1} floatIntensity={0.4} position={[11, -1.0, 17]}>
      ...
      {/* The Recliner - Rotated 180 degrees */}
      <LazyBoyChair position={[-0.8, 0.15, 0]} rotation={[0, Math.PI, 0]} />

      {/* Seated Resident (Bob) */}
      <group position={[-0.8, 0.62, 0]} rotation={[0, Math.PI, 0]}>
        <BlockHumanoid 
          scale={0.8} 
      ...
      {/* The L-Lamp - Shade over Bob's head */}
      <HomeLamp position={[-2.0, 0.14, 0]} />

      {/* Helper (Standing firmly on Platform) */}
