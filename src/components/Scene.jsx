...
const WheelchairChapter = ({ butterProps }) => {
  ...
  useFrame((state) => {
    ...
    if (t < 1) {
        if (wheelRef.current) wheelRef.current.rotation.x = state.clock.elapsedTime * 1.5; // SLOWER WHEEL
    } else if (isMoving) {
        setIsMoving(false);
    }
  });
...
const WalkingToConversationChapter = ({ butterProps }) => {
  ...
  useFrame((state) => {
    const t = Math.min(state.clock.elapsedTime * 0.03, 1); // SLOWER
    const startZ = 4.0;
    const endZ = 12.0; 
